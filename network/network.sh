...
## Bring up the network if it is not already up.
  ...
  scripts/createChannelMedDrop.sh $CHANNEL_NAME1 $CLI_DELAY $MAX_RETRY $VERBOSE
...
function deployCCOrder() {

  scripts/deployCCOrder.sh $CHANNEL_NAME1 $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE

  if [ $? -ne 0 ]; then
    fatalln "Deploying Order chaincode failed"
  fi
...
if [[ $# -ge 1 ]] ; then
  key="$1"
  if [[ "$key" == "createChannelMedDrop" ]]; then
      export MODE="createChannelMedDrop"
      shift    
  fi
fi
...
if [ "${MODE}" == "up" ]; then
  networkUp
elif [ "${MODE}" == "createChannelMedDrop" ]; then
  createChannelMedDrop
elif [ "${MODE}" == "deployCCOrder" ]; then
  deployCCOrder  
elif [ "${MODE}" == "down" ]; then
  networkDown
elif [ "${MODE}" == "restart" ]; then
